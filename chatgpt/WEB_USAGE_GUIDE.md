...
```text
请严格按照我上传的 Materials Text-Mining Introduction Writer instructions 工作。先判断论文类型，再默认输出 domain evidence card、reverse outline、narrative spine、中文 Introduction、claim-evidence map 和 quality-control table。如果评分低于 85，请先重写再输出最终版本。
```
...
- 如果你的论文不是普通抽取数据库论文，例如 RAG 文献综合、系统图、知识图谱或设计假设生成，要在输入中明确说明论文类型。
